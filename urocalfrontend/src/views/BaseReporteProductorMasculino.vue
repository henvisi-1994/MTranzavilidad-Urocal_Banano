...
export default {
  ...
  mounted() {
  this.cargarListaProductorPersonaM();
  },
  ...
  methods: {
    ...
    async download() {
      ...
      doc.setFontSize(14).setTextColor(0).text(88, 40, 'REPORTE DE PRODUCTORES MASCULINOS DE UROCAL'); 
      // Imagen Logo
    ...
    async cargarListaProductorPersonaM() {
      this.totalproductores = "0";
      let listaProductorPersona = []; // Limpiar la 'lista de datos'
      let respuesta = await ServicioReporteProductor.obtenerProductoresMasculino(); // Obtener respuesta de backend
