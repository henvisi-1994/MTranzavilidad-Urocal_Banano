<template>
  <v-dialog v-model="dialogNuevoReporteEmbarque" scrollable max-width="500px" transition="dialog-transition" :fullscreen="$vuetify.breakpoint.xs ? true : false">
    <v-card tile>
      <v-card-title class="justify-center primary white--text">
        <v-spacer></v-spacer>
        <h5>Nuevo Reporte Embarque</h5>
        <v-spacer></v-spacer>
      ...
      <v-card-actions>
        <v-col>
        <!--<v-btn color="error" block @click="dialogNuevoReporteEmbarque = !dialogNuevoReporteEmbarque" >Cancelar</v-btn>-->
...
export default {
  name: "DialogNuevoReporteEmbarque",
  ...
  computed: {
    ...
    ...mapState("moduloReporteEmbarque", ["reporte_embarque"]),   // Modulo Vehiculo
  ...
  methods: {
    cerrarDialogo(){
      this.dialogNuevoReporteEmbarque = !this.dialogNuevoReporteEmbarque;
