...
<!-- 
  ...
  -->
  <v-dialog v-model="dialogEditarConductorPersona" scrollable max-width="700px" transition="dialog-transition" :fullscreen="$vuetify.breakpoint.xs ? true : false">
    <v-card tile>
      ...
      <v-card-text>
        <v-row no-gutters>
          <v-col cols="12" md="6">
            <v-text-field :disabled="noeditar" v-model="modeloConductorPersonaStore.percedula" label="Cédula/RUC" class="custom px-2" dense filled></v-text-field>
          </v-col>
          <v-col cols="12" md="6">
            ...
            <template v-slot:activator="{ on, attrs }">
              <v-text-field :disabled="noeditar"
                ...
                :rules="[reglas.campoVacio(modeloConductorPersonaStore.perfechanacimiento)]"
                readonly
        ...
        <v-row no-gutters>
          <v-col cols="12" md="6">
            <v-text-field :disabled="noeditar" v-model="modeloConductorPersonaStore.pernombres" label="Nombres" class="custom px-2" dense filled></v-text-field>
          </v-col>
          <v-col cols="12" md="6">
            <v-text-field :disabled="noeditar" v-model="modeloConductorPersonaStore.perapellidos" label="Apellidos" class="custom px-2" dense filled></v-text-field>
          </v-col>
        </v-row>

        <v-row no-gutters> 
          <v-col cols="12" md="6">
            <v-text-field :disabled="noeditar" v-model="modeloConductorPersonaStore.pertelefono" label="Teléfono" class="custom px-2" dense filled></v-text-field>
          </v-col>
          <v-col cols="12" md="6">   
        ...
        <v-row no-gutters>
          <v-col cols="12" md="6">
            <v-text-field :disabled="noeditar" v-model="modeloConductorPersonaStore.perwhatsapp" label="Whatsapp" class="custom px-2" dense filled></v-text-field>
          </v-col>
          <v-col cols="12" md="6">
            <v-select :disabled="noeditar"
        ...
        <v-row no-gutters> 
          <v-col cols="12" md="6">
            <v-text-field :disabled="noeditar" v-model="modeloConductorPersonaStore.perdireccion" label="Dirección" class="custom px-2" dense filled></v-text-field>
          </v-col>
          <v-col cols="12" md="6">
            <v-text-field :disabled="noeditar" v-model="modeloConductorPersonaStore.conductorlicencia" label="Licencia" class="custom px-2" dense filled></v-text-field>                       
          </v-col>
        </v-row>
            
        <v-row no-gutters>
          <v-col cols="12" md="6">
            <v-text-field :disabled="noeditar" v-model="modeloConductorPersonaStore.peremail" label="Email" class="custom px-2" dense filled></v-text-field>
          </v-col>
          <v-col cols="12" md="6">
          </v-col>
        </v-row>
...
export default {
  ...
  methods: {
    ...
    async actualizarRegistro () { 
      ...
      const respuesta = await ServicioConductorPersona.actualizarConductorPersona(this.modeloConductorPersonaStore.personaid, this.modeloConductorPersonaStore);
    ...
    async eliminarRegistro() {
      //console.log(this.modeloConductorPersonaStore);
      const respuesta = await ServicioConductorPersona.eliminarConductorPersona(this.modeloConductorPersonaStore.personaid);
      if (respuesta.status == 200) {
        this.cerrarDialogo();
        this.cargarListaConductorPersona();
      } 
