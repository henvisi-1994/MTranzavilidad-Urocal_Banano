...
<!-- 
  ...
  -->
  <v-dialog v-model="dialogEditarProductorPersona" scrollable max-width="700px" transition="dialog-transition" :fullscreen="$vuetify.breakpoint.xs ? true : false">
    <v-card tile>

      <v-card-title class="primary--text">
        <h5 class="pl-3">Información del productor</h5>
        <v-spacer></v-spacer>
      ...
      <v-card-text>
        <v-row no-gutters>
          <v-col cols="12" md="6">
            <v-text-field :disabled="noeditar" v-model="modeloProductorPersonaStore.percedula" label="Cédula/RUC" class="custom px-2" dense filled></v-text-field>
          </v-col>
          <v-col cols="12" md="6">
            ...
            <template v-slot:activator="{ on, attrs }">
              <v-text-field :disabled="noeditar"
                ...
                :rules="[reglas.campoVacio(modeloProductorPersonaStore.perfechanacimiento)]"
                readonly
        ...
        <v-row no-gutters>
          <v-col cols="12" md="6">
            <v-text-field :disabled="noeditar" v-model="modeloProductorPersonaStore.pernombres" label="Nombres" class="custom px-2" dense filled></v-text-field>
          </v-col>
          <v-col cols="12" md="6">
            <v-text-field :disabled="noeditar" v-model="modeloProductorPersonaStore.perapellidos" label="Apellidos" class="custom px-2" dense filled></v-text-field>
          </v-col>
        </v-row>

        <v-row no-gutters> 
          <v-col cols="12" md="6">
            <v-text-field :disabled="noeditar" v-model="modeloProductorPersonaStore.pertelefono" label="Teléfono" class="custom px-2" dense filled></v-text-field>
          </v-col>
          <v-col cols="12" md="6">   
            <v-select :disabled="noeditar"
              v-model="modeloProductorPersonaStore.ciudadnacimiento.ciudadid" 
        ...
        <v-row no-gutters>
          <v-col cols="12" md="6">
            <v-text-field :disabled="noeditar" v-model="modeloProductorPersonaStore.perwhatsapp" label="Whatsapp" class="custom px-2" dense filled></v-text-field>
          </v-col>
          <v-col cols="12" md="6">
            <v-select :disabled="noeditar"
        ...
        <v-row no-gutters> 
          <v-col cols="12" md="6">
            <v-text-field :disabled="noeditar" v-model="modeloProductorPersonaStore.perdireccion" label="Dirección" class="custom px-2" dense filled></v-text-field>
          </v-col>
          <v-col cols="12" md="6">
            <v-text-field :disabled="noeditar" v-model="modeloProductorPersonaStore.peremail" label="Email" class="custom px-2" dense filled></v-text-field>
          </v-col>
        </v-row>

        <v-row no-gutters>
          <v-col cols="12" md="6">
            <v-text-field :disabled="noeditar" v-model="modeloProductorPersonaStore.productoridioma" label="Idioma" class="custom px-2" dense filled></v-text-field>            
          </v-col>
          <v-col cols="12" md="6">
          </v-col>
        </v-row>
...
export default {
  ...
  data() {
    return {
      ...
      itemsTipoUsuario: ['Productor', 'Responsable Centro Acopio', 'Productor', 'Administrador'],
    };
  },
  ...
  methods: {
    ...
    async actualizarRegistro () { 
      ...
      const respuesta = await ServicioProductorPersona.actualizarProductorPersona(this.modeloProductorPersonaStore.personaid, this.modeloProductorPersonaStore);
    ...
    async eliminarRegistro() {
      ...
      const respuesta = await ServicioProductorPersona.eliminarProductorPersona(this.modeloProductorPersonaStore.personaid);
      if (respuesta.status == 200) {
        this.cerrarDialogo();
        this.cargarListaProductorPersona();
      } 
