<template>
  <v-dialog v-model="dialogNuevoOperarioPersona" scrollable max-width="500px" transition="dialog-transition" :fullscreen="$vuetify.breakpoint.xs ? true : false">
    <v-card tile>
      <v-card-title class="justify-center primary white--text">
        <v-spacer></v-spacer>
        <h5>Nuevo Operario</h5>
        <v-spacer></v-spacer>
            ...
            <v-menu v-model="menuMostrarCalendario" transition="scale-transition" offset-y max-width="290px" min-width="290px">
              <template v-slot:activator="{ on, attrs }">
                <v-text-field
                  ...
                  v-model="operario_persona.perfechanacimiento" 
                  :rules="[reglas.campoVacio(operario_persona.perfechanacimiento)]"
                  readonly
                  v-bind="attrs"
                  v-on="on">
                </v-text-field>
              </template>
              <v-date-picker v-model="operario_persona.perfechanacimiento" @input="menuMostrarCalendario = false" :show-current="fechaActual" locale="es-419" ></v-date-picker>
            </v-menu>
      ...
      <v-card-actions>
        <v-col>
        <!--<v-btn color="error" block @click="dialogNuevoOperarioPersona = !dialogNuevoOperarioPersona" >Cancelar</v-btn>-->
...
export default {
  ...
  computed: {
    // Según el valor de la variable dialogNuevoOperarioPersona muestra u oculta el dialogo
    ...
    ...mapState("moduloOperarioPersona", ["operario_persona"]),   // Modulo OperarioPersona
