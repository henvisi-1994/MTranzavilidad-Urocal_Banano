...
module.exports = {
    ...
    async obtenerProductor(id) {
        let query = `SELECT p.personaid, p.percedula, p.perapellidos, p.pernombres, p.perdireccion, p.pertelefono, p.perwhatsapp, p.peremail, p.pergenero, 
                    extract(year from current_date) - extract(year from (SELECT TO_DATE(p.perfechanacimiento,'YYYY-MM-DD'))) AS edad, c.ciudadid, c.ciudadnombre, prod.productoridioma 
